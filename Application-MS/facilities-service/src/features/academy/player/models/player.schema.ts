...
playerSchema.pre(['findOneAndUpdate', 'updateOne', 'updateMany'], function (next) {
  this.set({ updatedAt: dayjs().unix() });
  next();
});

playerSchema.index({ academyId: 1, status: 1 });
playerSchema.index({ facilityId: 1, status: 1 });
...
const AcademyPlayerSchemaModel = mongoose.model('AcademyPlayer', playerSchema);
export default AcademyPlayerSchemaModel;
