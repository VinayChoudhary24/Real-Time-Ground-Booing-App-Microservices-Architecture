...
export const errorHandlerMiddleware: any = (
  ...
  const merchantUserId = (req.headers['x-user-id'] as string) || 'unauthenticated';
  ...
  if (err instanceof ErrorHandler) {
    ...
    err.message = err.message || 'Internal booking server error';
  ...
  return res.status(500).json({
    success: false,
    message: 'Something went wrong with booking server. Please try again later.',
  });
};
