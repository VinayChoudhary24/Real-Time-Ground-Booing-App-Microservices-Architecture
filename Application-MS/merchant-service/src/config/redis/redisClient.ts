...
export const connectRedis = () => {
  ...
  redis.on('connect', () => {
    consoleLogger.info({ service: 'Merchant-Redis', status: 'connected' });
  });

  redis.on('error', (err) => {
    consoleLogger.error({ service: 'Merchant-Redis', error: err.message });
  });
...
export const getRedis = () => {
  if (!redis) {
    throw new Error('Merchant-Redis is not initialized. Call connectRedis() first.');
  }
  return redis;
};
