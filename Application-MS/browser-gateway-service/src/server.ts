import { createServer } from "http";
import app from "./app.js";
import { appConfig } from "./config/appConfig/app.config";
import { green, red } from "console-log-colors";


const server = createServer(app);

const port = appConfig.port as number;
const host = appConfig.host;

server.listen(port, host, () => {
//   logger.info(`${appConfig.name} running in ${appConfig.host}:${appConfig.port}`)
console.log(green.bold(`service-name: ${appConfig.name}, running on: http://${appConfig.host}:${appConfig.port}`))
})

// Gracefully handle uncaught exceptions
process.on("uncaughtException", (err: Error) => {
  console.error(red.bold("UNCAUGHT EXCEPTION 🔥"), err);
  shutdownGracefully();
});

// Gracefully handle unhandled promise rejections
process.on("unhandledRejection", (reason: any, promise: Promise<any>) => {
  console.error(red.bold("UNHANDLED REJECTION ⚠️"), reason);
  shutdownGracefully();
});

// Graceful shutdown function
function shutdownGracefully() {
  console.log(red("Shutting down gateway service gracefully..."));
  server.close(() => {
    console.log(red("Server closed."));
    process.exit(1); // Exit with failure
  });

  // Force exit if shutdown takes too long
  setTimeout(() => {
    console.error(red("Force exiting..."));
    process.exit(1);
  }, 10000); // 10 seconds
}